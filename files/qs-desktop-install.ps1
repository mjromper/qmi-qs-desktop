...
Write-Host -Message "Downloading Qlik Sense Desktop binaries"
(New-Object System.Net.WebClient).DownloadFile($url, $dlLoc)

Write-Host -Message "Installing Qlik Sense Desktop..."
...
while (!(Test-Path "C:\Users\vagrant\AppData\Local\Programs\Qlik\Sense\QlikSense.exe")) {
    Write-Host -Message "Wait, Qlik Sense Desktop is being installed... next check in 60 secs"
    Start-Sleep -s 60
}

Write-Host -Message "Qlik Sense Desktop is being installed"